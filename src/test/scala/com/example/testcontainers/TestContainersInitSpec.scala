...
class TestContainersInitSpec extends SpecBase {
  ...
  val broker: ConfluentServerContainer = containerFactory
    .createConfluentServer()
    .withLogConsumer((frame: OutputFrame) => println(frame.getUtf8String.take(0)))
    .asInstanceOf[ConfluentServerContainer]
  val schemaRegistry: SchemaRegistryContainer =
    containerFactory.createSchemaRegistry(broker) //.withLogLevel("INFO")
  ...
  "must run simple topology on testcontainer server" in {
    ...
    val textLinesProducer = new KafkaProducer[Int, String](
      config,
      Serdes.intSerde.serializer(),
      Serdes.stringSerde.serializer
    )
    val translationsProducer = new KafkaProducer[String, String](
      config,
      Serdes.stringSerde.serializer(),
      Serdes.stringSerde.serializer
    )
    ...
    translationsEn foreach { case (w, tr) =>
      translationsProducer.send(
        new ProducerRecord[String, String](translationInputTopicName, w, tr)
      )
    }

    // TODO: check state and metrics here:

    val consumer = new KafkaConsumer[String, Translation](
      config,
  ...
  def makeTopology(): Topology = {
    ...
    val textLineStream: KStream[Int, String] =
      builder.stream(textLineInputTopicName)(Consumed.`with`(Serdes.intSerde, Serdes.stringSerde))
    ...
    val joined: KStream[String, Translation] =
      wordSplit.leftJoin(trTable)((_, translation) =>
        translation
      ) // TODO - whats the implicit here?
