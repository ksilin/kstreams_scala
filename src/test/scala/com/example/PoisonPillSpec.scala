...
class PoisonPillSpec extends SpecBase {
  ...
  val dataBeforePoisonPill: Seq[KeyValue[Integer, lang.Long]] = (1 to 3) map (i=> new KeyValue(i, i.longValue))
  val poisonPills: Seq[KeyValue[Integer, String]] = (4 to 6) map (i => new KeyValue(i, i.toString))
  val dataAfterPoisonPill: Seq[KeyValue[Integer, lang.Long]] = (7 to 9) map  (i => new KeyValue(i, i.longValue))
  ...
  "must process messages before and after the poison pills" in {
    ...
    streamsConfiguration.put(StreamsConfig.DEFAULT_DESERIALIZATION_EXCEPTION_HANDLER_CLASS_CONFIG, classOf[LogAndContinueExceptionHandler])
    ...
    outputRecords.asScala.values must contain theSameElementsAs(dataBeforePoisonPill.map(_.value) ++ dataAfterPoisonPill.map(_.value))

    // if you look in teh logs, you will see how the errors of the poison pills are logged:
  ...
  "custom DeserializationExceptionHandler - poison pill records mus land in the DLQ" in {
    ...
    streamsConfiguration.put(StreamsConfig.DEFAULT_DESERIALIZATION_EXCEPTION_HANDLER_CLASS_CONFIG, classOf[SendToDLQDeserializationExceptionHandler])
    val topologyTestDriver = new TopologyTestDriver(topology, streamsConfiguration)
