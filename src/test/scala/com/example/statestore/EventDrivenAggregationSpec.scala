...
class EventDrivenAggregationSpec extends SpecBase {
  ...
  val machineData: List[KeyValue[String, MachineData]] = machineIds flatMap {machineId =>
    ...
    val machineIdData = MachineData(machineId, machineId + "_customer", sensorData.map(_.value).asJava, now)
    (1 to 10).map(_.toString).toList map { _ =>
  ...
  "must aggregate and react to triggers" in {
    ...
    val inputTopicMachineData: TestInputTopic[String, MachineData] = topologyTestDriver.createInputTopic(
      inputTopicName,
