...
class TtlEmitterSpec extends SpecBase {
  ...
  def createTopology(builder: StreamsBuilder, inputTopic: String, outputTopic: String, storeName: String): Topology = {
    ...
    Stores.keyValueStoreBuilder(Stores.persistentKeyValueStore(purgeStoreName), intSerde, longSerde)
    ...
    val ttlEmitterTransformSupplier: TransformerSupplier[Int, MyRecord, KeyValue[Int, MyRecord]] = () => new TTLEmitter(Duration.ofMillis(ttl), punctuationInterval, purgeStoreName, PunctuationType.WALL_CLOCK_TIME)
    val storeTransformSupplier: TransformerSupplier[Int, MyRecord, KeyValue[Int, MyRecord]] = () => Transformers.storeAndDeleteTransformer(storeName)//.(punctuationInterval, ttl, tsExtractor, storeName)

    // writes tombstones back to input topic
    val ttlEmitter: Unit = input.transform(ttlEmitterTransformSupplier, purgeStoreName).to(inputTopicName)(Produced.`with`(intSerde, myRecordSerde))
  ...
  def prepTestData(driver: TopologyTestDriver, inputTopicName: String, outputTopicName: String, data: List[KeyValue[Integer, MyRecord]] ): TestOutputTopic[Integer, MyRecord] = {
    ...
    testInputTopic.pipeKeyValueList(data.asJava, Instant.ofEpochMilli(data.head.value.timestamp), Duration.ofMillis(1000))
    outputTopic
  }
