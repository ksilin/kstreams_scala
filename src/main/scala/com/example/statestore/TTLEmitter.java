...
public class TTLEmitter<K, V, R> implements Transformer<K, V, R> {
    ...
    public TTLEmitter(final Duration maxAge, final Duration scanInterval,
                      final String purgeStoreName, final PunctuationType punctuationType) {
        this.maxAge = maxAge;
        this.scanInterval = scanInterval;
        this.purgeStoreName = purgeStoreName;
    ...
    public void init(ProcessorContext context) {
        ...
        //
        context.schedule(scanInterval, punctuationType, timestamp -> {
            final long cutoff = timestamp - maxAge.toMillis();
            System.out.println("punctuating with cutoff " + cutoff);
            ...
            try (final KeyValueIterator<K, Long> all = stateStore.all()) {
                while (all.hasNext()) {
                    ...
                    if (record.value != null && record.value < cutoff) {
                        System.out.println("Forwarding tombstone for " + record.key);
                        // if a record's last update was older than our cutoff, emit a tombstone.
                        context.forward(record.key, null);
                    } else {
                        System.out.println("keeping record for " + record.key + " with ts " + record.value);
                    }
                }
            }
        });
